...
The reflection loop needs exactly one capability from a model: send messages,
get text back. It calls the model in three different roles — generator, critic
and reviser — but they are all the same method with different prompts.

Keeping the interface that narrow is what makes the pattern testable: the
rubric, the scoring, the pass rule, the "keep the best draft" policy and the
iteration cap are all our own Python, and a scripted fake can drive them end to
end. ``OpenAIClient`` is the live path and imports ``openai`` lazily, so
...
class FakeClient:
    ...
    that an iteration cap really stops a critic that never says "pass".
    """
