...
This pipeline needs exactly one capability from a model: send messages, get text
back. It calls the model in two roles — classifier and handler — but they are
the same method with different prompts.

Keeping the interface that narrow matters even more for guardrails than for the
other patterns, because the thing you most want to test is what happens when the
model misbehaves: a classifier that says something is in scope when it is not, a
handler that leaks a card number, a handler that cites a document that does not
exist. A scripted fake lets you write those cases as ordinary unit tests.
...
class FakeClient:
    ...
    that a repair cap really stops a handler that keeps returning bad output.
    """
